# Soccer Code Coach

Soccer Code Coach is a browser-based learning app for improving Java and Python
skills step by step. It explains core programming concepts through soccer terms:
variables are player lockers, loops are repeated training drills, functions are
set-piece routines, and debugging is match film review.

## What is included

- Step-by-step lessons for programming mindset, variables, conditionals, loops,
  collections, functions, object-oriented programming, and debugging.
- Side-by-side Java and Python examples for every concept.
- Soccer-themed explanations and practice drills.
- Interactive lesson completion tracking saved in the browser.
- Quick quiz questions with feedback.
- A glossary that translates soccer ideas into programming vocabulary.
- A repeatable study plan for continued practice.

## Run the app

Open `index.html` in a browser, or serve the folder with a simple local server:

```bash
cd soccer-code-coach
python3 -m http.server 8000
```

Then visit:

```text
http://localhost:8000
```

No install step is required. The app uses plain HTML, CSS, and JavaScript.

## Suggested learning flow

1. Start at Step 1 and read the soccer analogy before the code.
2. Type the Python example by hand, then switch to Java and type that version.
3. Mark the lesson complete after you can explain the idea in your own words.
4. Complete one practice drill for every two lessons.
5. Use the quiz as match review, then build your own small program from scratch.
